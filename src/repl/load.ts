...
export default async function (ctx: Context, _session: Session | null, opts: { name: string }) {
    ...
    if (!target.includes('.')) {
        ...
        if (root) {
            ...
            defineRootFn(runtimeOwner(ctx), target, m.default);
...
function runtimeOwner(ctx: Context): Context {
    let owner: any = ctx;
    while (owner && !Object.prototype.hasOwnProperty.call(owner, "state")) owner = Object.getPrototypeOf(owner);
    return owner ?? ctx;
}
